...
describe('Lottos 클래스 테스트', () => {
  describe('generateLottos 메서드는 numberOfLotto를 입력받아 구매 개수 만큼 로또 번호들을 반환한다.', () => {
    const cases = [
      {
        numbers: [
          [1, 2, 3, 4, 5, 6],
    ...
    test.each(cases)(
      '구매 개수 $numberOfLotto이 주어지는 경우, generateLottos()는 구매 개수 만큼 로또 번호들을 반환한다.',
      ({ numbers, numberOfLotto, expected }) => {
        // when
        mockRandoms(numbers);
        const lottos = new Lottos().generateLottos(numberOfLotto);
